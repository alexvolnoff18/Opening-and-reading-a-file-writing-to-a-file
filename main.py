...
def my_cook_book(text_file):
    """Функция возвращает словарь с рецептами из текстового файла со списком рецептов.         Рецепты в файле должны быть записаны в формате:
  'Название блюда
  ...
  ...'
    ...
    with open(text_file) as text:
        ...
        cook_book_dict = {n: [{k: v for (k, v) in zip(keys, map(str.strip,
                                                                next(lines).split(' | ', 2)))} for _ in
                              range(int(next(lines)))] for n in lines}
